...
def createImage(i, remove, folder):
    ...
    currentfile =  os.path.splitext(main.__file__)[0]
    ...
    outputfilename = folderpath + "/"+"Image"+str(i)+".png"
    # image.save(filename)
    inputfilename = folderpath + "/"+"Image"+str(i)+".eps"
    ps = cv.postscript(file=inputfilename,colormode = 'color')


def formatColor(r, g, b, a = 0.0):
...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    _root_window.protocol('WM_DELETE_WINDOW', raiseEndGraphics)
    ...
    _root_window.bind('<Control-Q>', raiseEndGraphics) 
    _root_window.bind_all('<Control-q>', raiseEndGraphics) 
...
def polygon(coords, outlineColor, fillColor=None, filled=1.0, smoothed=1, behind=0, width=1):
  c = []
  for coord in coords:
    c.append(coord[0])
    c.append(coord[1])
...
def square(pos, size, color, filled=1, behind=0):
    r = size / 2.0
    x = pos.x
    y = pos.y
    coords = [(x - r, y - r), (x + r, y - r), (x + r, y + r), (x - r, y + r)]
    borderColor = color
    return polygon(coords, borderColor, color, filled, 0, behind=behind)
...
def line(here, there, color=formatColor(0, 0, 0), width=2, dash = (4,4)):
  x0, y0 = here[0], here[1]
  x1, y1 = there[0], there[1]
  return _canvas.create_line(x0, y0, x1, y1, fill=color, width=width, dash = dash)
...
def color_change(object, color):
    _canvas.itemconfig(object, fill = color)
