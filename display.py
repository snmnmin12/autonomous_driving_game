...
class Display(object):
    ...
    @staticmethod
    def createImage(i, remove = False, folder = "Image"):
        graphicsUtils.createImage(i, remove, folder)
    @staticmethod
    ...
    # img.save(outputfilename, "png")
        ...
        for file in os.listdir(folderpath):
            if file.endswith(".eps"):
                filepath1 = os.path.join(folderpath, file)
                img = Image.open(filepath1)
                filename = os.path.splitext(file)[0]
                # filepath = os.path.join(folderpath, filename+".png")
                filepath = os.path.join(folderpath, "test.gif")
    ...
    def drawPath(lines):
        for i in range(len(lines)):
            if (i < len(lines)-1):
                 graphicsUtils.line(here=lines[i], there=lines[i+1],color= Display.RED, width=1, dash = (4,4))


    @staticmethod
    def drawLine(lines):
        for line in lines:
            graphicsUtils.line(here=line.getstart(), there=line.getend(),color= Display.BLACK, width=1, dash = (4,4))
    ...
    @staticmethod
    def colorchange(obj, color):
        #print 'attempt move'
        Display._acquireLock()
        #print 'move'
        parts = Display.partDict[obj]
        #assert(parts)
        graphicsUtils.color_change(parts, color)
        #print 'end move'
        Display._releaseLock()
